...
export default function page() {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const filteredProducts = response.data.products.filter(product => product.category === "FOOTWEAR" && product.department === "MENSWEAR");
  ...
  return (
    ...
    <div>
      ...
      <div className={style.wrapper2}>
        <span style={{ fontWeight: "bold" }}>{products.length} listings <span>{`MENS >FOOTWEAR `}</span></span>
